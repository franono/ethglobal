...
//SecretPayFactory, Produces the SecretPay agreements between users.
contract Secret_Factory{
    mapping (address => address[]) public SecretPayAddressesSeller;
    mapping (address => address[]) public SecretPayAddressesBuyer;
    
    //address public SecretPayAddress;
    event contractDeployed(
        address SecretPayAddress
    );
    
    //Creates, deploys and funds the SecretPay contract
    function createSecretPay(
        string _sellerKey,
        string _buyerKey,
        string[] _nodeKeys,
        address  _buyerAddress,
        ...
        address SecretPayAddress = (new Secret_SecretPay).value(address(this).balance)(
             _sellerKey,
             _buyerKey,
             _nodeKeys,
            msg.sender,
        ...
        SecretPayAddressesSeller[msg.sender].push(SecretPayAddress);
        SecretPayAddressesBuyer[_buyerAddress].push(SecretPayAddress);
    
        //Emit an event here\
        emit contractDeployed(SecretPayAddress);
    }
    
    function getSecretPayAddressesSeller() public constant returns(address[]) {
        return SecretPayAddressesSeller[msg.sender];
    }
    
    function getSecretPayAddressesBuyer() public constant returns(address[]) {
      return SecretPayAddressesBuyer[msg.sender];
    }
}